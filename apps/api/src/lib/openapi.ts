...
const documentation: GenerateSpecOptions["documentation"] = {
  ...
  info: {
    ...
    description: [
      ...
      "",
      "**Sessions and access.** Protected operations require `Authorization: Bearer <token>`.",
      "`/health`, the three `/session/*` operations, and five content reads are public; `/openapi.json`",
      "and `/docs` are public too. An unknown account is registered on first sign-in.",
      "`GET /me` reports the caller's role, and nine owner operations enforce it with `403 FORBIDDEN`.",
      "Public content reads omit management, processing, and personal read state for anonymous callers.",
      "",
      "**Entities, not roles.** Resources are the system's nouns: channels, episodes, runs, follows, the",
      "digest, the catalog. `?scope=all` includes channels in every status, and a channel's",
      "`management` block carries the fields the owner screens show when authorized.",
    ].join("\n"),
  },
  tags: [
  ...
  components: {
    securitySchemes: {
      session: {
        ...
        scheme: "bearer",
        description:
          "The session token from `POST /session/exchange`, after signing in through `GET /session/start`. Send it on protected operations; public operations also work without it.",
      },
    },
  },
  security: [{ session: [] }],
