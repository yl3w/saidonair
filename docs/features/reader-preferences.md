...
[PRD §7](../../docs/PRD.md) retains text treating chat rules as waiting for M4;
the field and answering integration are present. Its claim that the counts toggle
turns every count off is stronger than current call sites implement. Appearance
defaults and actual supported values above come from source, not inferred OS-theme
or account-sync behavior.
