...
“Starts only from Ask” describes the offered UI entry, not an API requirement.
Answer rendering linkifies both `youtube.com` and `youtu.be`. Retrieval requires
live Vectorize metadata indexes for `channelId` and `episodeId`; repository tests
do not verify those remote indexes.
