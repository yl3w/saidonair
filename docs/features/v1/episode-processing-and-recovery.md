...
`settleUnfinished` implements the deadline rule. Recovery currently has no per-tick
episode cap; staggering is the implemented throttle. The transcript adapter does
not generate missing captions. A failed replacement does not erase a working summary.
