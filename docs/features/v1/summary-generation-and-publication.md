...
Older 45-minute section comments are superseded by the executable 20-minute
constant. Partial parse failure can omit sections; an entirely raw fallback is
the first section's response, not a combined full-episode transcript. The shared
related-content query has no user filter; chat retrieval does, as described in
