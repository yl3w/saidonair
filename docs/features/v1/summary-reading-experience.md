...
The public reader offers `Aa`, navigation, and external links. Anonymous responses
omit `processing`, so runtime derived from that block is not available to the public
renderer. Return-to-row restoration depends on an anchor being present in the
loaded list. The executable Ask condition is documented above.
