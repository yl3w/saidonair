...
| A2–A3 | **The Registry re-key**, with its own spec and plan (`auth-2-registry-rekey.md`, owner instruction 2026-09-20): nine S/M steps converting one identity-bearing table at a time — schema column, module and callers together — because `email` stays `UNIQUE` so every foreign key survives the primary key moving. Ends in a full `wrangler dev` walkthrough on the old identity, the gate that proves the re-key is innocent. | working, header identity |
