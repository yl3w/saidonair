...
| A2 | **Registry schema and internals.** `0001_init.sql` edited in place: `user_id` PK, `email` nullable UNIQUE, `auth_user_id` nullable UNIQUE, the four audit and follower columns re-keyed. `ensureUser` mints ids; `seedOwner` unchanged. Callers still pass email. `/clean-local`. PRD §5.1, §5.4. | working, header identity |
| A3 | **Call sites.** `getUserDO(env, userId)`, `channel_followers`, `reviewed_by_user_id`, `skipped_by_user_id`, `requested_by_user_id`, the view modules resolving email for display, `/me` returning `userId`. **Ends in a full `wrangler dev` walkthrough on the old identity — the gate that proves the re-key is innocent.** | working, header identity |
