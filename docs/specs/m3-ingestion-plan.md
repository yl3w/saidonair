...
10–80 s before a Vectorize write is readable on a fresh index); M3.4 implemented the same day, awaiting commit: the
first product-visible chunk, approval and Start discover, the discovery cron runs in production. No new dependencies
anywhere in M3.
...
| M3.4 | `m3-4-discovery.md`, `-plan.md` | Step 7 discovery half, Step 8 discovery cron | M | M3.2 | yes: runs and pending episodes | implemented 2026-09-13, awaiting commit |
