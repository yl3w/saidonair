...
| Pre-flight gate | Every start, whoever asks for it, first reads DownSub's `/status` through the wrapper the catalog uses, cached for five minutes per isolate so the cost is one real call however many channels start. A 401 or `remainingCredits` of zero means nothing is launched and no run is recorded: cron logs `{ event: "ingestion.tick_skipped", reason }` and skips the tick, Start answers 502 `UPSTREAM_UNAVAILABLE` naming the reason, approve and the owner's add still approve and leave the channel "approved, never started", and retry still resets the episode to `pending`, starts nothing, and says so in its response (`start.outcome = "provider_blocked"`), since the episode leaves Needs attention on the reset and the owner must not read that as a run under way. An unreachable `/status` does not block. The gate applied to cron only until a reviewer noted the same day that a manual start with a known-bad key would cost every selected episode an attempt. `PROVIDER_LIMIT` (403, credits) met inside a run is a wait: `pending` with `waiting_code = PROVIDER_LIMIT`, run-episode `waiting`, no attempt. A run closes `completed` once every run-episode has an outcome; `ingestion_runs.failure_code` is never written. | A wrong key or an empty balance would otherwise burn one attempt per episode per tick. Checking once per tick, with a call that costs no credits, keeps both away from the episodes without a special outcome family. A key that breaks after the check reaches a few episodes as one attempt each, which the one rule handles. The catalog's `transcripts` block carries the check's answer so the owner sees a rejected key on the health strip. |
| Owner episode actions | **Retry accepts an episode in any state** (owner decision 2026-09-11): `POST /channels/:id/episodes/:videoId/retry` takes `failed`, `skipped`, `pending`, or `available` back to `pending` with attempts, wait, and skip fields cleared, and starts a one-episode `owner_retry` run. From `available` it re-fetches, re-embeds, and re-summarises: the existing summary leaves readers' view while the status is `pending` and is replaced when the new one lands; `processed_at` and read receipts are untouched, so the episode neither re-enters the digest nor shows NEW; vectors are overwritten by id, and any beyond the new chunk count are deleted in `shared-catalog` before publish; three failures leave it `failed` with the title only until the owner retries again or skips. From `pending` it forces an early re-check at one credit. `…/skip` stays `failed → skipped OWNER`. Both require an approved channel with no active run. Retry for `failed` and `skipped` was implemented on 2026-09-10; M3 widens it and makes it start a run. | We do not promise a better summary; we promise a retry. One rule for one button is simpler than a separate "re-summarise" action, and it is the only remedy the publish-without-a-gate decision needs. Siblings and their summaries are untouched. |
...
one instance per episode:
   ...
   trim           on a retry of an available episode only: vectors.deleteByIds(ns, ids from the new chunk count up
                  to the previous one), so no stale chunk outlives the new set
   publish        Registry.completeEpisode(runId, videoId, { chunkCount, summary, related }) → available,
                  processed_at set once, an existing summary row replaced, run-episode available
...
| Available | `status = 'available'`, `chunk_count`, `vectorized_at`, `processed_at` | | Retry to redo the summary; `processed_at` and receipts stay |
...
A feed that cannot be fetched leaves `last_checked_at` untouched and has two outcomes, both logged as
`{ event: "ingestion.feed_unavailable", channelId }`: with pending work, a normal `running` run whose run-episodes
are exactly the persisted `pending` episodes, since resuming them needs only their video ids; with nothing pending,
a run already `completed` with no run-episodes. A feed that was read but yielded nothing new, with nothing pending,
records the same empty run with `last_checked_at` moved. The `queued`, `failed`, and `cancelled` run statuses
...
4. A transport failure leaves the episode `pending` with `attempt_count` 1, the next run reattempts it, and the
   ...
   whose only run-episode is that video, leaving its siblings' summaries untouched. Retry on an `available` episode
   takes its summary out of readers' view, replaces it when the new one lands, deletes vectors beyond the new chunk
   count, and leaves `processed_at` and every read receipt exactly as they were; Retry on a `pending` episode
   re-fetches at once.
...
6. With a wrong key, the pre-flight check launches nothing from any start point: cron skips the tick, Start
   ...
   "approved, never started", retry leaves the episode `pending` with no run and answers 200 with
   `start.outcome = "provider_blocked"` so the owner sees it queued behind the key, and `GET /catalog` reports
