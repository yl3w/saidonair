...
| Pre-flight gate | Before a cron tick starts anything it calls DownSub's `/status` once through the same wrapper the catalog uses. A 401 or `remainingCredits` of zero means the tick launches nothing and logs `{ event: "ingestion.tick_skipped", reason }`; an unreachable `/status` does not block. `PROVIDER_LIMIT` (403, credits) met inside a run is a wait: `pending` with `waiting_code = PROVIDER_LIMIT`, run-episode `waiting`, no attempt. A run closes `completed` once every run-episode has an outcome; `ingestion_runs.failure_code` is never written. | A wrong key or an empty balance would otherwise burn one attempt per episode per tick. Checking once per tick, with a call that costs no credits, keeps both away from the episodes without a special outcome family. A key that breaks after the check reaches a few episodes as one attempt each, which the one rule handles. The catalog's `transcripts` block carries the check's answer so the owner sees a rejected key on the health strip. |
...
start point (approve · cron · Start · retry) ──► Worker handler, not a Workflow
   ...
   pre-flight (cron only)  DownSub /status; launch nothing on 401 or while remainingCredits = 0
   fetch-feed              rss.fetchChannelFeed(channelId); failure → a run is still recorded, already `completed`
                           with no run-episodes and last_checked_at untouched; cron and approve log it, Start also
                           answers 502 UPSTREAM_UNAVAILABLE; skipped for an owner retry, whose one video is already
                           tracked and may have left the feed
...
run-episode; if no run-episode is still `selected`, close the run `completed` and set channels.last_ingested_at
when at least one became available.
```
...
| Cron finds nothing new and nothing pending, or cannot read the feed | An empty `completed` run is recorded; `last_checked_at` moves only when the feed was read | None; the next tick looks again. Runs that keep appearing while `last_checked_at` stands still are how an unreadable feed shows |
...
1. First approval and the owner's on-demand start each create one run row with its selected run-episodes and one
   ...
   `last_checked_at`; a start while YouTube does not answer records an empty `completed` run, leaves
   `last_checked_at` alone, and answers 502; re-approving a declined channel that had been approved before starts
   nothing.
...
6. With a wrong key, the tick's pre-flight check launches nothing and `GET /catalog` reports `transcripts.status`
   as `auth_failed`; an instance that meets a 401 after the check counts one attempt on its episode like any other
   error; fixing the key lets the next tick run and picks every episode up.
7. `wrangler dev --test-scheduled` plus a request to `/__scheduled` starts runs for approved, unpaused channels
   ...
   tick; one whose feed cannot be read records the same run with `last_checked_at` unmoved.
...
9. The first cron after approval imports no feed entry published before the channel's `approved_at`; a new upload
   after it is imported. A channel for which no run has yet selected an episode gets an `initial` run from Start or
